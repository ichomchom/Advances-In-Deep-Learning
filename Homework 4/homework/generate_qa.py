...
def draw_detections(
    ...
    for detection in frame_detections:
        ...
        draw.rectangle([(x1_scaled, y1_scaled), (x2_scaled,
                       y2_scaled)], outline=color, width=thickness)
...
def extract_kart_objects(
    ...
    # Identify the kart closest to the image center
    if karts:
        image_center = (img_width // 2, img_height // 2)
        closest_kart = min(karts, key=lambda k: np.linalg.norm(
            np.array(k["center"]) - np.array(image_center)))
        closest_kart["is_center_kart"] = True
        karts = [closest_kart] + [k for k in karts if k["instance_id"]
                                  != closest_kart["instance_id"]]
    return karts
...
def generate_qa_pairs(info_path: str, view_index: int, img_width: int = 150, img_height: int = 100) -> list:
    ...
    # Counting questions
    left_count = sum(1 for k in karts if k["instance_id"] !=
                     ego_kart["instance_id"] and k["center"][0] < ego_kart["center"][0])
    right_count = sum(1 for k in karts if k["instance_id"] !=
                      ego_kart["instance_id"] and k["center"][0] >= ego_kart["center"][0])
    front_count = sum(1 for k in karts if k["instance_id"] !=
                      ego_kart["instance_id"] and k["center"][1] < ego_kart["center"][1])
    behind_count = sum(1 for k in karts if k["instance_id"] !=
                       ego_kart["instance_id"] and k["center"][1] >= ego_kart["center"][1])
...
# def generate_all_qa_pairs(data_dir: str, output_dir: str = "data/train"):
...
#         frame_id, _ = extract_frame_info(
#             str(info_file.name.replace("_info.json", "_00_im.png")))
...
#                 output_filename = f"{frame_id:05d}_{view_index:02d}_qa_pairs.json"
#                 with open(out_path / output_filename, "w") as f_out:
#                     json.dump(qa_pairs, f_out, indent=2)
...
def main():
    ...
    # fire.Fire({"generate": generate_all_qa_pairs})


if __name__ == "__main__":
    main()
    # print(draw_detections("../data/valid/00095_03_im.jpg",
    #                       "../data/valid/00095_info.json"))
