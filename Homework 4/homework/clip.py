...
def clip_data_collator(features: list[dict[str, torch.Tensor]]) -> dict[str, torch.Tensor]:
    ...
    input_ids = torch.stack([pad_tensor(
        f["input_ids"], pad_value=processor.tokenizer.eos_token_id) for f in features])
    attention_mask = torch.stack(
        [pad_tensor(f["attention_mask"], pad_value=0) for f in features])
    pixel_values = torch.stack([f["pixel_values"]
                               for f in features])  # assume all are same shape
    labels = torch.stack(
        [pad_tensor(f["labels"], pad_value=-100) for f in features])
...
class CLIP(nn.Module):
    def __init__(
        ...
        self.image_projection = nn.Linear(
            vision_encoder.config.hidden_size, proj_dim)
        self.text_projection = nn.Linear(
            text_encoder.config.hidden_size, proj_dim
        )
    ...
    def encode_image(self, image: torch.Tensor) -> torch.Tensor:
        ...
        # Ensure output matches projection layer dtype
        return features.to(self.image_projection.weight.dtype)

    def encode_text(self, input_ids: torch.Tensor, attention_mask: torch.Tensor = None) -> torch.Tensor:
        outputs = self.text_encoder(
            input_ids=input_ids, attention_mask=attention_mask)
        if attention_mask is not None:
            masked_embeddings = outputs.last_hidden_state * \
        ...
        # Ensure output matches projection layer dtype
        return features.to(self.text_projection.weight.dtype)
    ...
    def save_pretrained(self, save_directory: str, **kwargs):
        ...
        torch.save(additional_state_dict, Path(
            save_directory) / "additional_weights.pt")

    def load_pretrained(self, load_directory: str, **kwargs):
        """Customize load method, load projection additional parameters"""

        additional_weights_path = Path(
            load_directory) / "additional_weights.pt"
...
def compute_clip_loss(
    ...
    """
    image_proj, text_proj, logits = outputs

    # Create labels for contrastive learning (diagonal should be positive pairs)
    batch_size = logits.shape[0]
    targets = torch.arange(batch_size, device=logits.device)

    # Compute cross-entropy loss for both directions (image->text and text->image)
    loss_i_to_t = F.cross_entropy(logits, targets)
    loss_t_to_i = F.cross_entropy(logits.T, targets)
...
def test(ckpt_path: str, val_dataset: str = "valid_grader"):
    ...
    for pair in tqdm.tqdm(testset):
        ...
        pixel_values = image_processor(
            image).unsqueeze(0).to(device).bfloat16()
        ...
        prediction = torch.matmul(
            vision_feature, text_feature.T).argmax(dim=-1)
