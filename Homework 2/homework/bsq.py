...
class BSQPatchAutoEncoder(PatchAutoEncoder, Tokenizer):
    ...
    def encode_index(self, x: torch.Tensor) -> torch.Tensor:
        ...
        ae_encoded = self.encoder(patches)
        return self.bsq.encode_index(ae_encoded)

    def decode_index(self, x: torch.Tensor) -> torch.Tensor:
        bsq_decoded = self.bsq.decode_index(x)
        ae_decoded = self.decoder(bsq_decoded)
        return self.unpatchify(ae_decoded)

    def encode(self, x: torch.Tensor) -> torch.Tensor:
        patches = self.patchify(x)
        ae_encoded = self.encoder(patches)
        return self.bsq.encode(ae_encoded)

    def decode(self, x: torch.Tensor) -> torch.Tensor:
        bsq_decoded = self.bsq.decode(x)
        ae_decoded = self.decoder(bsq_decoded)
        return self.unpatchify(ae_decoded)
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        reconstructed = self.unpatchify(decoded)
        cnt = torch.bincount(self.encode_index(x).flatten(),
        ...
        return reconstructed, usage_stats
